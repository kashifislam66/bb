$(document).ready(function() {		
	...
	var xin_table_currencies = $('#xin_table_currencies').dataTable({
		...
		"ajax": {
            url : main_url+"settings/currency_list",
            type : 'GET'
        },
		"language": {
            "lengthMenu": dt_lengthMenu,
	...
	// listing
	var xin_table_subscription_tax = $('#xin_table_subscription_tax').dataTable({
		"bDestroy": true,
		//"bFilter": false,
		//"bLengthChange": false,
		"iDisplayLength": 10,
		"aLengthMenu": [[10, 30, 50, 100, -1], [10, 30, 50, 100, "All"]],
		"ajax": {
            url : main_url+"types/subscription_tax_list",
            type : 'GET'
        },
		"language": {
            "lengthMenu": dt_lengthMenu,
	...
	jQuery("#subscription_tax_info").submit(function(e){
	/*Form Submit*/
	e.preventDefault();
		var obj = jQuery(this), action = obj.attr('name');
		$('.icon-spinner3').show();
		jQuery.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=28&data=subscription_tax_info&type=subscription_tax_info&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					toastr.error(JSON.error);
					...
					xin_table_subscription_tax.api().ajax.reload(function(){ 
						toastr.success(JSON.result);
					}, true);
					$('input[name="csrf_token"]').val(JSON.csrf_hash);
					jQuery('#subscription_tax_info')[0].reset(); // To reset form fields
					Ladda.stopAll();
				}
			}
		});
	...
	if(tk_type == 'payment_method'){
		...
		var field_add = '&type=delete_record&';
		var tb_name = 'xin_table_'+tk_type;
	} else if(tk_type == 'subscription_tax'){
		var field_add = '&type=delete_record&';
		var tb_name = 'xin_table_'+tk_type;
	}
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		} else if(field_type == 'subscription_tax'){
			var field_add = '&data=ed_subscription_tax&type=ed_subscription_tax&';
		} else if(field_type == 'cms_pages'){
