$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
        "bDestroy": true,
		"ajax": {
            url : main_url+"transfers/transfers_list",
            type : 'GET'
        },
		"language": {
	...
	jQuery("#employee_id").change(function(){
		jQuery.get(main_url+"transfers/is_department/"+jQuery(this).val(), function(data, status){
	...
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		var modal = $(this);
	$.ajax({
		url : main_url+"transfers/read_transfer",
		type: "GET",
...
$( document ).on( "click", ".delete", function() {
	...
	$('#delete_record').attr('action',main_url+'transfers/delete_transfer');
});