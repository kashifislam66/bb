...
class Announcements extends BaseController {
	...
	{		
		...
		$data['path_url'] = 'announcement_view';
	...
	public function announcement_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {
			$audience_id = explode(',',$r['audience_id']);
			$department_id = explode(',',$r['department_id']);
			if((in_array($employee_detail['department_id'], $department_id) OR in_array('all', $department_id)) AND (in_array($usession['sup_user_id'], $audience_id) OR in_array('all', $audience_id)) OR $user_info['user_type'] != 'staff'){
			/* get Employee info*/
			if(in_array('all',$audience_id)) {
				$ol = lang('Main.xin_all_audience');
			} else {
				$ol = '';
				foreach(explode(',',$r['audience_id']) as $uid) {
					$iuser_info = $UsersModel->where('user_id', $uid)->first();
					if($iuser_info){
						$ol .= $iuser_info['first_name'].' '.$iuser_info['last_name'].'<br>';
					} else {
						$ol .= '';
					}
				 }
			...
			if(in_array('all',$department_id)) {
				$dol = lang('Main.xin_all_departments');
			} else {
				$dol = '';
				foreach(explode(',',$r['department_id']) as $uid) {
					$department = $DepartmentModel->where('department_id', $uid)->first();
					if($department){
						$dol .= $department['department_name'].'<br>';
					} else {
						$dol .= '';
					}
				 }
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$data = [
					...
					'announcement_comment'  => $description,
					'created_at' => date('d-m-Y h:i:s')
				];
				$AnnouncementcommentsModel = new AnnouncementcommentsModel();
				$result = $AnnouncementcommentsModel->insert($data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Main.xin_annc_comment_added_success');
				} else {
					$Return['error'] = lang('Main.xin_error_msg');
				}
	...
	public function get_department_employees()
	{
		...
		if($session->has('sup_username')){
			return view('erp/announcements/get_department_employees', $data);
		} else {
			return redirect()->to(site_url('erp/login'));
		}
	}
	...
	// delete record
	public function delete_announcement_comment() {
		...
		if($this->request->getVar('field_id')) {
			...
			$Return['csrf_hash'] = csrf_hash();
			$AnnouncementcommentsModel = new AnnouncementcommentsModel();
			$result = $AnnouncementcommentsModel->where('comment_id', $id)->delete($id);
			if ($result == TRUE) {
				$Return['result'] = lang('Main.xin_annc_comment_deleted_success');
			} else {
				$Return['error'] = lang('Main.xin_error_msg');
			}
			$this->output($Return);
