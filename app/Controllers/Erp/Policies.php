...
class Policies extends BaseController {
	...
	public function policies_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {
			...
			$download_link = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_download_signed_file').'"><a href="'.site_url().'download?type=policy&filename='.uencode($r['attachment']).'"><button type="button" class="btn icon-btn btn-sm btn-light-dark waves-effect waves-light"><i class="feather icon-download"></i></button></a></span>';
			
			$created_at = set_date_format($r['created_at']);
			$added_by = $UsersModel->where('user_id', $r['added_by'])->first();			
			$title = $r['title'];
			$combhr = $download_link.$edit.$delete;
	...
	// read record
	public function read_policy_flipbook()
	{
		$session = \Config\Services::session();
		$request = \Config\Services::request();
		if(!$session->has('sup_username')){ 
		...
		if($session->has('sup_username')){
			return view('erp/policies/dialog_policy_flipbook', $data);
		} else {
			return redirect()->to(site_url('erp/login'));
		}
	}
