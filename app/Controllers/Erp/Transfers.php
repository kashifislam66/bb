...
class Transfers extends BaseController {
	...
	{		
		...
		$isegment_val = $TransfersModel->where('transfer_id', $ifield_id)->first();
		if(!$isegment_val){
			$session->setFlashdata('unauthorized_module',lang('Dashboard.xin_error_unauthorized_module'));
			return redirect()->to(site_url('erp/desk'));
		}
		...
		if($user_info['user_type']=='staff'){
			...
			if($status == 0){
				update_notifications_read_status_pending('transfer_settings',$ifield_id);
			} else {
				update_notifications_read_status_approved('transfer_settings',$ifield_id);
			}
		}
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Main.xin_transfer_details').' | '.$xin_system['application_name'];
		$data['path_url'] = 'transfers';
		$data['breadcrumbs'] = lang('Main.xin_transfer_details');

		$data['subview'] = view('erp/transfers/key_transfer_details', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	// |||add record|||
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				if($iskip_specific == 1){
					...
					$result = $TransfersModel->where('transfer_id', $id)->first();
					$old_level = staff_reporting_manager_old_level($result['employee_id'],'transfer_settings');
					$total_levels = $old_level['total_levels'];
				}
				// check approvals
				$approval_cnt = $StaffapprovalsModel->where('company_id', $company_id)->where('module_key_id', $id)->where('module_option', 'transfer_settings')->countAllResults();
