...
class Types extends BaseController {
	...
	{		
		...
		if($user_info['user_type'] != 'company'){
			if(!in_array('incident_type1',staff_role_resource())) {
				$session->setFlashdata('unauthorized_module',lang('Dashboard.xin_error_unauthorized_module'));
				return redirect()->to(site_url('erp/desk'));
			}
		}
		$data['title'] = lang('Main.xin_incident_types').' | '.$xin_system['application_name'];
		$data['path_url'] = 'cons_incident_type';
		$data['breadcrumbs'] = lang('Main.xin_incident_types');

		$data['subview'] = view('erp/constants/key_incident_type', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	...
	// record list
	public function subscription_tax_list() {
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		$category = $ConstantsModel->where('type','subscription_tax')->orderBy('constants_id', 'ASC')->findAll();
		$data = array();
		
          foreach($category as $r) {
			  
			$edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_edit').'"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_type="subscription_tax" data-field_id="'. uencode($r['constants_id']) . '"><i class="feather icon-edit"></i></button></span>';
			
			$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.lang('Main.xin_delete').'"><button type="button" class="btn icon-btn btn-sm btn-light-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-token_type="subscription_tax" data-record-id="'. uencode($r['constants_id']) . '"><i class="feather icon-trash-2"></i></button></span>';
			...
			if($r['field_two']=='fixed'){
				...
				$field_one = $r['field_one'].'%';
				$_type = lang('Employees.xin_title_tax_percent');
			}
			if($r['constants_id']==181){
				$combhr = $edit;
			} else {
				$combhr = $edit.$delete;
			}
	...
	public function incident_type_list() {
		...
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
			$category = $ConstantsModel->where('company_id',$user_info['company_id'])->where('type','incident_type')->orderBy('constants_id', 'ASC')->findAll();
		} else {
			$category = $ConstantsModel->where('company_id',$usession['sup_user_id'])->where('type','incident_type')->orderBy('constants_id', 'ASC')->findAll();
		}
		$data = array();
		
          foreach($category as $r) {
			  
			if(in_array('incident_type3',staff_role_resource()) || $user_info['user_type'] == 'company') { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_edit').'"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-field_id="'. uencode($r['constants_id']) . '"><i class="feather icon-edit"></i></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('incident_type4',staff_role_resource()) || $user_info['user_type'] == 'company') { //edit
				$delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="'.lang('Main.xin_delete').'"><button type="button" class="btn icon-btn btn-sm btn-light-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. uencode($r['constants_id']) . '"><i class="feather icon-trash-2"></i></button></span>';
			} else {
				$delete = '';
			}
			...
			if(in_array('incident_type3',staff_role_resource()) || in_array('incident_type4',staff_role_resource()) || $user_info['user_type'] == 'company') {
				$category_name = '
					'.$r['category_name'].'
					<div class="overlay-edit">
						'.$combhr.'
	...
	// read record
	public function read_incident_type()
	{
		$session = \Config\Services::session();
		$request = \Config\Services::request();
		if(!$session->has('sup_username')){ 
		...
		if($session->has('sup_username')){
			return view('erp/constants/dialog_incident_type', $data);
		} else {
			return redirect()->to(site_url('erp/login'));
		}
	}
	...
	public function add_product_category() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Main.xin_product_category_added_success');
				} else {
	...
	public function add_incident_type() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Success.ci_incident_type_added_msg');
				} else {
					$Return['error'] = lang('Main.xin_error_msg');
				}
				$this->output($Return);
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$data = [
					...
					'category_name' => $name,
					'type'  => 'leave_type',
					'field_one'  => 'a:6:{s:8:"is_carry";s:1:"0";s:11:"carry_limit";s:1:"0";s:17:"is_negative_quota";s:1:"0";s:14:"negative_limit";s:1:"0";s:8:"is_quota";s:1:"1";s:12:"quota_assign";a:50:{i:1;s:3:"105";i:2;s:3:"105";i:3;s:3:"112";i:4;s:3:"119";i:5;s:3:"126";i:6;s:3:"113";i:7;s:1:"0";i:8;s:1:"0";i:9;s:1:"0";i:10;s:1:"0";i:11;s:1:"0";i:12;s:1:"0";i:13;s:1:"0";i:14;s:1:"0";i:15;s:1:"0";i:16;s:1:"0";i:17;s:1:"0";i:18;s:1:"0";i:19;s:1:"0";i:20;s:1:"0";i:21;s:1:"0";i:22;s:1:"0";i:23;s:1:"0";i:24;s:1:"0";i:25;s:1:"0";i:26;s:1:"0";i:27;s:1:"0";i:28;s:1:"0";i:29;s:1:"0";i:30;s:1:"0";i:31;s:1:"0";i:32;s:1:"0";i:33;s:1:"0";i:34;s:1:"0";i:35;s:1:"0";i:36;s:1:"0";i:37;s:1:"0";i:38;s:1:"0";i:39;s:1:"0";i:40;s:1:"0";i:41;s:1:"0";i:42;s:1:"0";i:43;s:1:"0";i:44;s:1:"0";i:45;s:1:"0";i:46;s:1:"0";i:47;s:1:"0";i:48;s:1:"0";i:49;s:1:"0";i:50;s:1:"0";}}',
	...
	public function add_competencies() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				$ruleErrors = [
                    "name" => $validation->getError('name'),
					//"comments" => $validation->getError('comments'),
	...
	// |||add record|||
	public function add_subscription_tax() {
			
		$validation =  \Config\Services::validation();
		$session = \Config\Services::session();
		$request = \Config\Services::request();
		$usession = $session->get('sup_username');	
		if ($this->request->getPost('type') === 'subscription_tax_info') {
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = csrf_hash();
			// set rules
			$rules = [
	...
	public function update_leave_type() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$id = udecode($this->request->getPost('token',FILTER_SANITIZE_STRING));	
				//extra leave options
				$extra_options = $this->request->getPost('extra_options',FILTER_SANITIZE_STRING);
				$iextra_options = serialize($extra_options);
	...
	// |||update record|||
	public function update_competencies_type() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Main.xin_competency_updated_success');
				} else {
					$Return['error'] = lang('Main.xin_error_msg');
				}
				$this->output($Return);
	...
	public function close_year($company_id ="") {
	$res = 	checkCarryBalnce($company_id);
