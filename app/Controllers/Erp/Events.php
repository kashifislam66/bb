...
class Events extends BaseController {
	...
	public function add_event() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				if(preg_match("/((1[0-2]|0?[1-9]):([0-5][0-9]) ?([AaPp][Mm]))/", $event_time) == false) {
				...
				$is_show_calendar = $this->request->getPost('is_show_calendar',FILTER_SANITIZE_STRING);
				...
				$data = [
					...
					'is_show_calendar'  => $is_show_calendar,
	...
	// read record
	public function add_new_event_record()
	{
		$session = \Config\Services::session();
		$request = \Config\Services::request();
		if(!$session->has('sup_username')){ 
