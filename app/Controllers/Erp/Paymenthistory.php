	...
	public function payment_history_list()
     {
		...
		$data = array();
          ...
          foreach($billing as $r) {						
			...
			if($r['payment_method'] == 'Stripe'){
			...
			} else if($r['payment_method'] == 'PayPal'){
				...
				$invoice_url = site_url('erp/payment-details').'/'.uencode($r['membership_invoice_id']);
				$logo = '<i class="fas fa-money-check-alt text-success"></i>';
				$target_blnk = '';
				$membership_price = $r['membership_price'];
				$price = number_to_currency($membership_price, $xin_system['default_currency'],null,2);
			}
