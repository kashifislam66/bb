...
use App\Models\NotificationstatusModel;
...
class Complaints extends BaseController {
	...
	{		
		...
		if($user_info['user_type']=='staff'){
			...
			if($status == 0){
				update_notifications_read_status_pending('complaint_settings',$ifield_id);
			} else {
				update_notifications_read_status_approved('complaint_settings',$ifield_id);
			}
		}
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Main.xin_view_complaint').' | '.$xin_system['application_name'];
		$data['path_url'] = 'complaints';
		$data['breadcrumbs'] = lang('Main.xin_view_complaint');

		$data['subview'] = view('erp/complaints/complaints_details', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	// |||edit record|||
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				if($iskip_specific == 1){
					...
					$result = $ComplaintsModel->where('complaint_id', $id)->first();
				...
				// check approvals
				$approval_cnt = $StaffapprovalsModel->where('company_id', $company_id)->where('module_key_id', $id)->where('module_option', 'complaint_settings')->countAllResults();
					...
					if($iapproval_cnt == $total_levels){	
						...
						if($iapproval_cnt < $total_levels){
							if($status == 2) {
								...
								$ComplaintsModel = new ComplaintsModel();
								$ComplaintsModel->update($id, $data);
								$data2 = [
									'company_id'  => $company_id,
									'staff_id'  => $usession['sup_user_id'],
									'module_key_id'  => $id,
	...
	public function complaints_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {
			...
			$view = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_view_details').'"><a href="'.site_url().'erp/view-complaint-info/'.uencode($r['complaint_id']).'"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>';
