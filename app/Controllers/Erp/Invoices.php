		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$data = [
					...
					'deposit_to'  => 0,
					'with_holding_taxid'  => 0,
					'with_holding_tax'  => 0,
					'pay_full'  => 0,
					'total_amount'  => $fgrand_total,
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				$ruleErrors = [
					...
					"payment_date" => $validation->getError('payment_date'),
					"deposit_to" => $validation->getError('deposit_to'),
					"with_holding_tax" => $validation->getError('with_holding_tax'),
					"status" => $validation->getError('status'),
					"total_amount" => $validation->getError('total_amount')
                ];
				foreach($ruleErrors as $err){
					$Return['error'] = $err;
					if($Return['error']!=''){
				...
				$deposit_to = $this->request->getPost('deposit_to',FILTER_SANITIZE_STRING);
				$with_holding_tax = $this->request->getPost('with_holding_tax',FILTER_SANITIZE_STRING);
				$pay_full = $this->request->getPost('pay_full',FILTER_SANITIZE_STRING);
				$status = $this->request->getPost('status',FILTER_SANITIZE_STRING);
				$total_amount = $this->request->getPost('total_amount',FILTER_SANITIZE_STRING);
				$holding_tax_rate = $this->request->getPost('holding_tax_rate',FILTER_SANITIZE_STRING);
				$id = udecode($this->request->getPost('token',FILTER_SANITIZE_STRING));	
				if($pay_full == '' || $pay_full == 0){
					$pay_full = 0;
				} else {
					$pay_full = $pay_full;
				}
				...
				$data = [
					...
					'with_holding_taxid'  => $with_holding_tax,
					'with_holding_tax'  => $holding_tax_rate,
					'pay_full'  => $pay_full,
