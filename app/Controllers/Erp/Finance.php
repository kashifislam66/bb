...
class Finance extends BaseController {
	...
	{		
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Finance.xin_accounts').' | '.$xin_system['application_name'];
		$data['path_url'] = 'finance_new_accounts';
		$data['breadcrumbs'] = lang('Finance.xin_accounts');

		$data['subview'] = view('erp/finance/finance_new_accounts', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	...
	// record list
	public function new_accounts_list() {
		...
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
			$get_data = $StaffaccountsModel->where('company_id',$user_info['company_id'])->orderBy('account_id', 'ASC')->findAll();
		} else {
			$get_data = $StaffaccountsModel->where('company_id',$usession['sup_user_id'])->orderBy('account_id', 'ASC')->findAll();
		}
		$data = array();
	...
	public function add_new_account() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$StaffaccountsModel = new StaffaccountsModel();
				$result = $StaffaccountsModel->insert($data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Success.ci_finance_account_added_msg');
				} else {
	...
	// |||edit record|||
	public function update_new_account() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$StaffaccountsModel = new StaffaccountsModel();
				$result = $StaffaccountsModel->update($id,$data);	
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
					$Return['result'] = lang('Success.ci_finance_account_updated_msg');
				} else {
	...
	// read record
	public function read_new_accounts()
	{
		$session = \Config\Services::session();
		$request = \Config\Services::request();
		if(!$session->has('sup_username')){ 
		...
		if($session->has('sup_username')){
			return view('erp/finance/dialog_new_accounts', $data);
		} else {
			return redirect()->to(site_url('erp/login'));
		}
	}
	...
	// delete record
	public function delete_new_account() {
		...
		if($this->request->getPost('type')=='delete_record') {
			...
			$result = $StaffaccountsModel->where('account_id', $id)->where('company_id', $company_id)->delete($id);
			if ($result == TRUE) {
				$Return['result'] = lang('Success.ci_finance_account_deleted_msg');
			} else {
				$Return['error'] = lang('Main.xin_error_msg');
