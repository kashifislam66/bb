...
class Travel extends BaseController {
	...
	{		
		...
		if($user_info['user_type']=='staff'){
			...
			if($status == 0){
				update_notifications_read_status_pending('travel_settings',$ifield_id);
			} else {
				update_notifications_read_status_approved('travel_settings',$ifield_id);
			}
		}
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				if($iskip_specific == 1){
					...
					$result = $TravelModel->where('travel_id', $id)->first();
					$old_level = staff_reporting_manager_old_level($result['employee_id'],'travel_settings');
					$total_levels = $old_level['total_levels'];
				}
				// check approvals
				$approval_cnt = $StaffapprovalsModel->where('company_id', $company_id)->where('module_key_id', $id)->where('module_option', 'travel_settings')->countAllResults();
