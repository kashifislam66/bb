...
class Employees extends BaseController {
	...
	{		
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Main.xin_employees_directory').' | '.$xin_system['application_name'];
		$data['path_url'] = 'employees_grid';
		$data['breadcrumbs'] = lang('Main.xin_employees_directory');
	...
	public function staff_set_salary()
	{		
		$RolesModel = new RolesModel();
		$UsersModel = new UsersModel();
		$SystemModel = new SystemModel();
		...
		$usession = $session->get('sup_username');
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Main.xin_set_salary').' | '.$xin_system['application_name'];
		$data['path_url'] = 'employee_details';
		$data['breadcrumbs'] = lang('Main.xin_set_salary');

		$data['subview'] = view('erp/employees/staff_set_salary', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	public function staff_profile_list()
	{		
		$RolesModel = new RolesModel();
		$UsersModel = new UsersModel();
		$SystemModel = new SystemModel();
		...
		$usession = $session->get('sup_username');
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Main.xin_staff_profile_list').' | '.$xin_system['application_name'];
		$data['path_url'] = 'employee_details';
		$data['breadcrumbs'] = lang('Main.xin_staff_profile_list');

		$data['subview'] = view('erp/employees/staff_profile_list', $data);
	...
	public function add_employee() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			// count employees
			$count_staff = $UsersModel->where('company_id', $company_id)->where('user_type', 'staff')->countAllResults();
			...
			if($count_staff >= $subs_plan['total_employees']) {
				$Return['error'] = lang('Employees.xin_upgrade_your_subscription_add_more_employees');
			}
			if($Return['error']!=''){
				$this->output($Return);
			}
		...
		//	$validated = $this->request->getPost('file',FILTER_SANITIZE_STRING);
			if($_FILES['file']['size'] == 0) {
				...
				$file_name = $user_image->getRandomName();
				$user_image->move('public/uploads/users/',$file_name);
			...
			$not_part_of_orgchart = $this->request->getPost('not_part_of_orgchart',FILTER_SANITIZE_STRING);
			$not_part_of_system_reports = $this->request->getPost('not_part_of_system_reports',FILTER_SANITIZE_STRING);
			$hourly_rate = 0;//$this->request->getPost('hourly_rate',FILTER_SANITIZE_STRING);
			$salay_type = 1;//$this->request->getPost('salay_type',FILTER_SANITIZE_STRING);
			...
			$data2 = [
				...
				'ml_eis_contribution' => 0,
				'ml_socso_category'  => 0,
				'ml_pcb_socso'  => 0,
				'ml_hrdf'  => 0,
				
				'ml_tax_citizenship'  => 0,
				'zakat_fund'  => 0,
				'job_type' => 1,
				'assigned_hours' => '',
				'leave_options' => '',
				...
				'approval_level03' => 0,
				'not_part_of_orgchart' => $not_part_of_orgchart,
				'not_part_of_system_reports' => $not_part_of_system_reports,
				'is_accrual_pause' => 0,
				'pause_start_date' => '',
				'pause_start_end' => '',
				'created_at' => date('d-m-Y h:i:s')
	...
	public function add_document() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$rules = [
				...
				'document_file' => [
					'rules'  => 'uploaded[document_file]|ext_in[document_file,jpg,jpeg,png,pdf,doc,docx]|max_size[document_file,6072]',
					'errors' => [
	...
	public function update_basic_info() {
		...
		$user_id = udecode($this->request->getPost('token',FILTER_SANITIZE_STRING));
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$validation->setRules([
					...
					'employee_id' => 'required',			
					'role' => 'required',
					'kiosk_code' => 'required|numeric|min_length[4]|max_length[4]|is_unique[ci_erp_users.kiosk_code,user_id,'.$user_id.']',
				],
				[   // Errors
					'first_name' => [
						'required' => lang('Main.xin_employee_error_first_name'),
			...
			if ($validation->hasError('first_name')) {
				...
				$Return['error'] = $validation->getError('role');
			} elseif($validation->hasError('kiosk_code')){
				$Return['error'] = $validation->getError('kiosk_code');
			}
			if($Return['error']!=''){
				$this->output($Return);
			}
			...
			$data = [
				...
				'gender' => $gender,
				'is_active'  => $status,
				'kiosk_code' => $this->request->getPost('kiosk_code')
			];
	...
	public function update_contract_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$is_contract_end = $this->request->getPost('is_contract_end',FILTER_SANITIZE_STRING);
	...
	// |||update record|||
	public function update_malaysia_salary() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			// set rules
			//staff details
			$ml_tax_category = $this->request->getPost('ml_tax_category',FILTER_SANITIZE_STRING);
			$ml_empployee_epf_rate = $this->request->getPost('ml_empployee_epf_rate',FILTER_SANITIZE_STRING);
			$ml_empployer_epf_rate = $this->request->getPost('ml_empployer_epf_rate',FILTER_SANITIZE_STRING);
			$ml_eis_contribution = $this->request->getPost('ml_eis_contribution',FILTER_SANITIZE_STRING);
			$ml_socso_category = $this->request->getPost('ml_socso_category',FILTER_SANITIZE_STRING);
			$ml_hrdf = $this->request->getPost('ml_hrdf',FILTER_SANITIZE_STRING);	
			$ml_tax_citizenship = $this->request->getPost('ml_tax_citizenship',FILTER_SANITIZE_STRING);		
			$zakat_fund = $this->request->getPost('zakat_fund',FILTER_SANITIZE_STRING);
			$id = udecode($this->request->getPost('token',FILTER_SANITIZE_STRING));
			// employee details
			$data2 = [
				'ml_tax_category'  => $ml_tax_category,
				'ml_empployee_epf_rate'  => $ml_empployee_epf_rate,
				'ml_empployer_epf_rate'  => $ml_empployer_epf_rate,
				'ml_eis_contribution' => $ml_eis_contribution,
				'ml_socso_category'  => $ml_socso_category,
				'ml_hrdf'  => $ml_hrdf,
				'ml_tax_citizenship'  => $ml_tax_citizenship,
			...
			$Return['csrf_hash'] = csrf_hash();	
			if ($result == TRUE) {
				$Return['result'] = lang('Main.xin_ml_payroll_values_updated_msg');
			} else {
				$Return['error'] = lang('Main.xin_error_msg');
			}
			$this->output($Return);
	...
	public function leave_options() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			// set rules
			//staff details
			$leave_opt = $this->request->getPost('leave_opt',FILTER_SANITIZE_STRING);
			$assigned_hours = $this->request->getPost('assigned_hours',FILTER_SANITIZE_STRING);
			$is_accrual_pause = $this->request->getPost('is_accrual_pause',FILTER_SANITIZE_STRING);
			$pause_start_date = $this->request->getPost('pause_start_date',FILTER_SANITIZE_STRING);
			$pause_start_end = $this->request->getPost('pause_start_end',FILTER_SANITIZE_STRING);
			$id = udecode($this->request->getPost('token',FILTER_SANITIZE_STRING));
			$ileave_opt = serialize($leave_opt);
			$iassigned_hours = serialize($assigned_hours);
			
			if($is_accrual_pause == 1){
				$cistart_date = strtotime($pause_start_date);
				$ciend_date = strtotime($pause_start_end);
				if($pause_start_date === ''){
					$Return['error'] = lang('Main.xin_start_accrual_date_error');
				} else if($pause_start_end === ''){
					$Return['error'] = lang('Main.xin_end_accrual_date_error');
				} else if($cistart_date > $ciend_date){
			...
			// employee details
			$data2 = [
				'leave_options'  => $ileave_opt,
				'assigned_hours'  => $iassigned_hours,
				'is_accrual_pause'  => $is_accrual_pause,
				'pause_start_date'  => $pause_start_date,
				'pause_start_end'  => $pause_start_end,
			];
			$MainModel = new MainModel();
			$result = $MainModel->update_employee_record($data2,$id);
			$Return['csrf_hash'] = csrf_hash();	
			if ($result == TRUE) {
				$Return['result'] = lang('Main.xin_leave_accruals_updated');
			} else {
				$Return['error'] = lang('Main.xin_error_msg');
			}
			$this->output($Return);
