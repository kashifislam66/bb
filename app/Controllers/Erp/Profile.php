...
class Profile extends BaseController {
	...
	public function system_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$validation->setRules([
				...
				[   // Errors
					...
					'is_lunch_break' => [
						'required' => 'Lunch Break Field Required',
					]
				]
			);
			$validation->withRequest($this->request)->run();
			...
			}elseif($validation->hasError('is_lunch_break')){
				$Return['error'] = $validation->getError('is_lunch_break');
			}
			if($Return['error']!=''){
				$this->output($Return);
			}
		...
		$ip_address = $this->request->getPost('ip_address',FILTER_SANITIZE_STRING);	
		$ml_payroll = $this->request->getPost('ml_payroll',FILTER_SANITIZE_STRING);
		$enable_ip_address = $this->request->getPost('enable_ip_address',FILTER_SANITIZE_STRING);	
		$default_language = $this->request->getPost('default_language',FILTER_SANITIZE_STRING);	
		$bank_account = $this->request->getPost('bank_account',FILTER_SANITIZE_STRING);	
		$vat_number = $this->request->getPost('vat_number',FILTER_SANITIZE_STRING);	
		$hrm_staff_dashboard = $this->request->getPost('hrm_staff_dashboard',FILTER_SANITIZE_STRING);	
		$invoice_terms_condition = $this->request->getPost('invoice_terms_condition',FILTER_SANITIZE_STRING);
		$is_lunch_break = $this->request->getPost('is_lunch_break',FILTER_SANITIZE_STRING);
		$hide_org_name = $this->request->getPost('hide_org_name',FILTER_SANITIZE_STRING);
		$hide_org_photo = $this->request->getPost('hide_org_photo',FILTER_SANITIZE_STRING);
		...
		$data = [
			...
			'is_enable_ml_payroll'  => $ml_payroll,
			'default_language'  => $default_language,
			'bank_account'  => $bank_account,
			'vat_number'  => $vat_number,
			'hrm_staff_dashboard'  => $hrm_staff_dashboard,
			'invoice_terms_condition'  => $invoice_terms_condition,
			'is_lunch_break'  => $is_lunch_break,
			'hide_org_name'  => $hide_org_name,
			'hide_org_photo'  => $hide_org_photo,
        ];
		$MainModel = new MainModel();
