...
class Tasks extends BaseController {
	...
	{		
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Dashboard.left_tasks').' | '.$xin_system['application_name'];
		$data['path_url'] = 'general_tasks';
		$data['breadcrumbs'] = lang('Dashboard.left_tasks');

		$data['subview'] = view('erp/projects/general_tasks', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	...
	public function general_tasks_grid()
	{		
		$RolesModel = new RolesModel();
		$UsersModel = new UsersModel();
		$SystemModel = new SystemModel();
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Dashboard.left_tasks').' | '.$xin_system['application_name'];
		$data['path_url'] = 'general_tasks_grid';
		$data['breadcrumbs'] = lang('Dashboard.left_tasks');

		$data['subview'] = view('erp/projects/general_tasks_grid', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	...
	public function general_task_details()
	{		
		$RolesModel = new RolesModel();
		$UsersModel = new UsersModel();
		$SystemModel = new SystemModel();
		...
		$data['subview'] = view('erp/projects/general_task_details', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
	...
	public function tasks_list() {
		...
		if($user_info['user_type'] == 'staff'){
			...
			$get_data = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->orderBy('task_id', 'ASC')->findAll();
		}
		$data = array();
	 ...
	 // record list
	public function general_tasks_list() {
		...
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
			$get_data = assigned_staff_general_tasks($usession['sup_user_id']);
		} else {
			$get_data = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->orderBy('task_id', 'ASC')->findAll();
		}
		$data = array();
          ...
          foreach($get_data as $r) {
			...
			$view = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="'.lang('Main.xin_view_details').'"><a href="'.site_url('erp/general-task-detail').'/'.uencode($r['task_id']).'"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light"><i class="feather icon-arrow-right"></i></button></a></span>';
			//assigned user
			$assigned_to = explode(',',$r['assigned_to']);
			$multi_users = multi_user_profile_photo($assigned_to);
	...
	public function add_task() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$task_type = $this->request->getPost('task_type',FILTER_SANITIZE_STRING);
	...
	public function task_status_chart() {
		...
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
			$get_tasks = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->countAllResults();
			$not_started = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->where('task_status', 0)->countAllResults();
			$in_progress = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->where('task_status', 1)->countAllResults();
			$completed = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->where('task_status', 2)->countAllResults();
			$cancelled = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->where('task_status', 3)->countAllResults();
			$hold = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','project')->where('task_status', 4)->countAllResults();
		} else {
			$get_tasks = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->countAllResults();
			$not_started = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->where('task_status', 0)->countAllResults();
			$in_progress = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->where('task_status', 1)->countAllResults();
			$completed = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->where('task_status', 2)->countAllResults();
			$cancelled = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->where('task_status', 3)->countAllResults();
			$hold = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','project')->where('task_status', 4)->countAllResults();
		}
		/* Define return | here result is used to return user data and error for error message */
