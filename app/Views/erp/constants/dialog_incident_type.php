...
$get_animate = '';
if($request->getGet('data') === 'incident_type' && $request->getGet('field_id')){
$category_id = udecode($field_id);
$result = $ConstantsModel->where('constants_id', $category_id)->where('type','incident_type')->first();
?>

<div class="modal-header">
  <h5 class="modal-title">
    <?= lang('Main.xin_edit_incident_type');?>
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="name" placeholder="<?= lang('Main.xin_incident_type');?>" value="<?= $result['category_name'];?>">
      </div>
    </div>
  </div>
</div>
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					var xin_table = $('#xin_table').dataTable({
						"bDestroy": true,
						"ajax": {
							url : "<?= site_url("erp/types/incident_type_list") ?>",
							type : 'GET'
						},
