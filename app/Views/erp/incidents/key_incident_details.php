...
$result = $IncidentsModel->where('incident_id', $ifield_id)->first();
$category_info = $ConstantsModel->where('company_id', $company_id)->where('type','incident_type')->findAll();
// get approvals data
$approval_cnt = $StaffapprovalsModel->where('company_id', $company_id)->where('staff_id', $usession['sup_user_id'])->where('module_key_id', $ifield_id)->where('module_option', 'incident_settings')->countAllResults();
$approvals_data = $StaffapprovalsModel->where('company_id', $company_id)->where('module_key_id', $ifield_id)->where('module_option', 'incident_settings')->findAll();
  ...
  <div class="col-xl-4 col-lg-12 task-detail-right">
    <div class="card">
      <div class="card-header">
        <h5><?= lang('Main.xin_update_incident_status');?></h5>
      ...
      <?php if($result['status'] != 2){ ?>
      	  <?php if($approval_cnt < 1) {?>
              ...
              <?php echo form_open('erp/incidents/update_status_record', $attributes, $hidden);?>
              <?php } ?>
              <div class="card-body">
    ...
    <div class="card">
      <div class="card-header">
        <h5 class=""><i class="fas fa-clock m-r-5"></i>
          <?= lang('Main.xin_incident_details');?>
        </h5>
      </div>
        ...
        <div class="table-responsive">
              <table class="table m-b-0 f-14 b-solid requid-table">
                <tbody class="text-muted">
                  <tr>
                    <td><?= lang('Main.xin_incident_type');?></td>
                    <td>
                      <?php foreach($category_info as $_category) {?>
                      <?php if($_category['constants_id']==$result['incident_type_id']):?> <?= $_category['category_name']?> <?php endif;?>
                      <?php } ?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Dashboard.xin_title');?></td>
                    <td class="">
                      <?= $result['title'];?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Main.xin_incident_date');?></td>
                    <td class="">
                      <?= set_date_format($result['incident_date']);?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Main.xin_time');?></td>
                    <td><?= $result['incident_time'];?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Main.xin_incident_document');?></td>
                    <td class="">
                    <?php if($result['incident_file']!='' && $result['incident_file']!='no file') {?>
                    <a href="<?= site_url()?>download?type=incidents&filename=<?= uencode($result['incident_file']);?>">
