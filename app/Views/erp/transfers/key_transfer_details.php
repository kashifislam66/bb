...
// get approvals data
$approval_cnt = $StaffapprovalsModel->where('company_id', $company_id)->where('staff_id', $usession['sup_user_id'])->where('module_key_id', $ifield_id)->where('module_option', 'transfer_settings')->countAllResults();
$approvals_data = $StaffapprovalsModel->where('company_id', $company_id)->where('module_key_id', $ifield_id)->where('module_option', 'transfer_settings')->findAll();

$result = $TransfersModel->where('transfer_id', $ifield_id)->first();
$departments = $DepartmentModel->where('company_id',$company_id)->orderBy('department_id', 'ASC')->findAll();
$designations = $DesignationModel->where('company_id',$company_id)->where('department_id',$result['transfer_department'])->orderBy('designation_id', 'ASC')->findAll();
...
if($iskip_specific == 1){
	...
	$old_level = staff_reporting_manager_old_level($result['employee_id'],'transfer_settings');
  ...
  <div class="col-xl-4 col-lg-12 task-detail-right">
    <div class="card">
      <div class="card-header">
        <h5><?= lang('Main.xin_update_transfer_status');?></h5>
      ...
      <?php if($result['status'] != 2){ ?>
      	  <?php if($approval_cnt < 1) {?>
              ...
              <?php echo form_open('erp/transfers/update_status_record', $attributes, $hidden);?>
              <?php } ?>
              <div class="card-body">
    ...
    <div class="card">
      <div class="card-header">
        <h5 class=""><i class="fas fa-clock m-r-5"></i>
          <?= lang('Main.xin_transfer_details');?>
        </h5>
      </div>
      ...
      <div class="card-body">
        <div class="table-responsive">
              <table class="table m-b-0 f-14 b-solid requid-table">
                <tbody class="text-muted">
                  <tr>
                    ...
                    <?php if($staff['user_id']==$result['employee_id']):?> <?= $staff['first_name'].' '.$staff['last_name'] ?> <?php endif;?>
                    <?php } ?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Employees.xin_transfer_department');?></td>
                    <td class="">
                      <?php foreach($departments as $idepartment) {?>
					<?php if($employee_detail['department_id'] != $idepartment['department_id']){?>
                    <?php if($idepartment['department_id']==$result['transfer_department']):?> <?= $idepartment['department_name']?> <?php endif;?>
                    <?php } ?>
                    <?php } ?></td>
                  </tr>
                  <tr>
                    <td><?= lang('Employees.xin_transfer_designation');?></td>
                    <td class="">
                      <?php foreach($designations as $idesignation) {?>
                      <?php if($idesignation['designation_id']==$result['transfer_designation']):?> <?= $idesignation['designation_name']?> <?php endif;?>
                        <?php } ?></td>
                  <tr>
                    <td><?= lang('Employees.xin_transfer_date');?></td>
                    <td class="">
                      <?= set_date_format($result['transfer_date']);?></td>
                  </tr>
                  <?php
				   //notify_send_to
            ...
            <div class="m-b-30 m-t-15">
              <h6><?= lang('Employees.xin_transfer_reason');?></h6>
              <hr>
