...
if($user_info['user_type'] == 'staff'){
	...
	$get_tasks = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where("assigned_to like '%$id,%' or assigned_to like '%,$id%' or assigned_to = '$id'")->paginate(9);
	$total_tasks = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->orderBy('task_id', 'ASC')->countAllResults();
	$not_started = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where('task_status', 0)->countAllResults();
	$in_progress = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where('task_status', 1)->countAllResults();
	$completed = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where('task_status', 2)->countAllResults();
	$cancelled = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where('task_status', 3)->countAllResults();
	$hold = $TasksModel->where('company_id',$user_info['company_id'])->where('task_type','general')->where('task_status', 4)->countAllResults();
	...
	$get_tasks = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->orderBy('task_id', 'ASC')->paginate(9);
	$total_tasks = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->orderBy('task_id', 'ASC')->countAllResults();
	$not_started = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->where('task_status', 0)->countAllResults();
	$in_progress = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->where('task_status', 1)->countAllResults();
	$completed = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->where('task_status', 2)->countAllResults();
	$cancelled = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->where('task_status', 3)->countAllResults();
	$hold = $TasksModel->where('company_id',$usession['sup_user_id'])->where('task_type','general')->where('task_status', 4)->countAllResults();
	$pager = $TasksModel->pager;
}
?>
<div class="row"> 
  ...
  <div class="col-xl-12 col-lg-12 filter-bar">
    <nav class="navbar m-b-30 p-10">
      ...
      <div class="nav-item nav-grid f-view"> <span class="m-r-15">
        ...
        :</span> <a href="<?= site_url().'erp/general-tasks-list';?>" class="btn btn-sm waves-effect waves-light btn-primary btn-icon m-0" data-toggle="tooltip" data-placement="top" title="<?= lang('Projects.xin_list_view');?>"> <i class="fas fa-list-ul"></i> </a> <a href="<?= site_url().'erp/general-tasks-grid';?>" class="btn btn-sm waves-effect waves-light btn-primary btn-icon m-0" data-toggle="tooltip" data-placement="top" title="<?= lang('Projects.xin_grid_view');?>"> <i class="fas fa-th-large"></i> </a>
        ...
        <div id="accordion">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <input type="hidden" value="general" name="task_type" />
              <div class="col-md-3">
                <div class="form-group">
    ...
    <div class="row">
      ...
      <div class="col-md-4 col-sm-12">
        <div class="card card-border-c-blue">
          <div class="card-header"> <a href="<?= site_url('erp/general-task-detail').'/'.uencode($r['task_id']);?>" class="text-secondary">#
            <?= $i;?>
            .
            <?= $r['task_name'];?>
          ...
          <div class="card-body card-task">
            ...
            <div class="task-board" style="float: inherit;">
              <div class="dropdown-secondary dropdown"> <a href="<?= site_url('erp/general-task-detail').'/'.uencode($r['task_id']);?>">
                <?= $status?>
                </a> </div>
              <div class="dropdown-secondary dropdown"> <a href="<?= site_url('erp/general-task-detail').'/'.uencode($r['task_id']);?>">
              ...
              <div class="dropdown-secondary dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdown3" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut"> <a class="dropdown-item" href="<?= site_url('erp/general-task-detail').'/'.uencode($r['task_id']);?>"> <i class="feather icon-eye"></i>
                  <?= lang('Projects.xin_view_task');?>
                  </a>
                  <?php if(in_array('task2',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                  <a class="dropdown-item" href="<?= site_url('erp/general-task-detail').'/'.uencode($r['task_id']);?>?type=edit"> <i class="feather icon-edit"></i>
