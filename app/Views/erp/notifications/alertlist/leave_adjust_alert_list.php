  ...
  <div class="col-xl-12 col-md-12">
    <div class="card marketing-card table-card">
      <div class="card-header">
        <h5>
          <?= lang('Main.xin_leave_adjust_alert_list');?>
        </h5>
      </div>
        ...
        <div class="card-body p-b-0">
          <div class="table-responsive">
            <table class="table table-hover mb-0">
              ...
              <tbody>
                <?php
                $com_alert = staff_approval_notification_leave_adjust_info('leave_adjustment_settings');
				// New Approvals
				if($iskip_specific == 1){
					if($level_option['total_levels'] > 1){
						if($level_option['level1']==$usession['sup_user_id']){
							foreach($com_alert['leave_adjust_alert_data'] as $_leave_arr):
							foreach($_leave_arr as $_leave):
								...
								$check_levels
								 ...
								 ->where('module_key_id', $_leave['adjustment_id'])
							...
							<tr>
							  <td><?= $name;?></td>
							  <td><?= $itype_name;?></td>
							  <td><?= $_leave['adjust_hours'];?></td>
							  <td><?= $applied_on;?></td>
							  <td><a href="<?= site_url().'erp/view-leave-adjustment/'.uencode($_leave['adjustment_id']);?>">View Request</a><?= $make_approve.$change_status;?></td>
							</tr>
							<?php
							...
							foreach($_leave_arr as $_leave):
								...
								// if($check_levels > 0){
								 $approval_data_cnt = $StaffapprovalsModel->where('company_id',$company_id)
									...
									->where('staff_id',$level_option['level1'])
									 ->where('module_key_id', $_leave['adjustment_id'])
									->where('module_option','leave_adjustment_settings')->where('status',1)->countAllResults();
								// status
								if($check_levels < 1) {
									$change_status = '';
									$make_approve = '<br><span class="badge bg-light-primary">My pending approval</span>';
								...
								if($approval_data_cnt > 0){
									$link = '<a href="'.site_url().'erp/view-leave-adjustment/'.uencode($_leave['adjustment_id']).'">View Request</a>'.$make_approve;
								} else {
					...
					foreach($com_alert['leave_adjust_alert_data'] as $_leave_arr):
						foreach($_leave_arr as $_leave):
							$old_level = staff_reporting_manager_old_level($_leave['employee_id'],'leave_adjustment_settings');
